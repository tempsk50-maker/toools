...
import { Camera, Loader2, Sparkles, Upload, Trash2, RefreshCcw, Type, Check, Settings, Star, Copy, Newspaper, CheckCircle2, Ratio, Plus, Image as ImageIcon } from 'lucide-react';
...
// UPDATED TEMPLATES LIST - ALL 14 DESIGNS
...
const NewsCardGenerator: React.FC<NewsCardGeneratorProps> = ({ 
  ...
  const [defaultTemplate, setDefaultTemplate] = useState<NewsCardTemplate>(() => {
    try {
      ...
      return cardType === 'quote' ? 'bk-quote-grid-classic' : 'bk-news-classic-light';
    } catch {
      return cardType === 'quote' ? 'bk-quote-grid-classic' : 'bk-news-classic-light';
    }
  });
  ...
  useEffect(() => {
    // Reset selection if switching between News/Quote modes
  ...
  useEffect(() => {
    ...
    setInputText('');
    setImages([]);
    // Default to transparent for quotes as per new designs preference
  ...
  // Optimized Image Upload: Uses Blob URLs for instant preview
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (!files || files.length === 0) return;

    // Create Blob URLs immediately - no async/await needed for preview
  ...
  const handleResize = useCallback(() => {
      if (containerRef.current) {
        ...
        // Base width of the canvas is always 600px
  ...
  const handleCapture = async () => {
    ...
    const width = 600;
    // Determine height based on card type: Quote = 750 (4:5), News = 600 (1:1)
    const height = cardType === 'quote' ? 750 : 600;
    ...
    try {
        ...
        canvas.toBlob((blob: Blob | null) => {
            ...
            a.href = url;
            a.download = `bk-${cardType}-${Date.now()}.png`;
  ...
  const isQuote = cardType === 'quote';
  const inputPlaceholder = isQuote 
    ? 'উদাহরণ: "ছাত্ররা যদি জেগে ওঠে তবে কেউ তাদের থামাতে পারবে না" - ড. ইউনূস' 
    : 'এখানে দীর্ঘ, অগোছালো বা বায়াসড টেক্সট পেস্ট করুন। AI এটি ঠিক করে দিবে...';
  const imageLabel = isQuote ? 'বক্তার ছবি' : 'নিউজ ছবি';
  
  // Dynamic Height for Container
  const baseHeight = isQuote ? 750 : 600;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-2 lg:gap-6 animate-in fade-in duration-300 relative">
      ...
      <div className="lg:col-span-5 space-y-2 lg:space-y-4 order-1">
        ...
        <div className="bg-bk-surface-light dark:bg-bk-surface-dark p-3 lg:p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-bk-border-dark transition-all">
           ...
           {/* Enhanced Compact Upload Area - Line Style */}
           ...
           {/* Common Settings for both News and Quote */}
        ...
        <div className="bg-bk-surface-light dark:bg-bk-surface-dark p-3 lg:p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-bk-border-dark text-sm transition-all">
           ...
           <div className="space-y-2 lg:space-y-3">
                <textarea
                   ...
                   placeholder={isQuote ? "মূল উক্তি লিখুন" : "হেডলাইন লিখুন"}
                />
                {isQuote && (
                   <input 
                      type="text"
      ...
      <div className="lg:col-span-7 flex flex-col items-center order-2 mt-0 lg:mt-0">
         <div className="sticky top-24 space-y-2 lg:space-y-4 w-full flex flex-col items-center">
            ...
            {/* Canvas Area (Preview) - FIXED LAYOUT & SCALING */}
            <div 
              ...
              ref={containerRef}
            >
                {/* Scaled Wrapper that reserves the correct layout space */}
                <div style={{ 
                    width: scale ? 600 * scale : '100%',
                    height: scale ? baseHeight * scale : 'auto',
                    position: 'relative',
                    transition: 'width 0.2s, height 0.2s'
                }}>
                    {/* The Canvas Content - Scaled from Top Left */}
                    <div style={{ 
                        width: '600px',
